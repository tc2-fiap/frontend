...
React + Vite + TypeScript. Talks to every backend through one relative base URL (`/api/*`) — the Ingress routes by path, so this app never needs a service-specific port or host. See `../documentation/spec/instructions.md` §7.
...
Deployed by the `orchestration` Helm chart alongside the five backend services — see `repos/orchestration`.
...
- `src/utils/currency.ts` — `formatPrice(value, currency)` (BRL via `pt-BR`/`Intl.NumberFormat`, USD via `en-US`, defaulting to BRL) and `brlToUsd()`. Every backend price is still a plain BRL number — `src/hooks/useQuotation.ts` fetches `catalog-api`'s live rate once per page load and the catalog page converts for display only when the toggle is English, degrading to native BRL if the rate is unavailable.
